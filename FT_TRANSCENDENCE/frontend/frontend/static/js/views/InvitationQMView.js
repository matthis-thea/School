...
export default class extends AbstractView {
	constructor() {
		super();
		this.setTitle("InvitationQM");
	}

	async getHtml() {
		try {
			// Load changepass.html
			const response = await fetch('static/html/invitationQM.html');
			if (!response.ok) {
			...
			script.type = 'module';
			script.src = `./static/js/InvitationQM.js?${new Date().getTime()}`;
